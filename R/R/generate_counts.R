...
generate_counts <- function(count_files_dir, file_ext_to_remove, remove_last_rows=FALSE,rows_to_remove=4) {
  ...
  for (f in countfiles ) {
    x <- as.data.frame( read.table(paste0(count_files_dir,'/',f), row.names=1) )
